...
{-# LANGUAGE StrictData #-}
module Solver.Tactics.Blast.BabyBlasts.IntersectionBlast.IntersectionBlast where
...
import Solver.Tactics.Blast.BabyBlasts.IntersectionBlast.Features
...
import qualified Solver.Tactics.Blast.BabyBlasts.IntersectionBlast.Features as Features
...
buildIntersectionBlastCtx :: Ex (Grid Color) -> ForTrain (Grid Color) -> Synth1Context
buildIntersectionBlastCtx inputs outputs =
...
intersectionBlast :: StdGoal -> SolveM TacticResult
intersectionBlast tacGoal@(TacGoal.Goal inputs outputs ctx)  = do
  guard $ all (uncurry Grid.sameDims) (zip (Ex.train inputs) outputs)
  let reconstructs = initialReconstructs inputs outputs
  let blastCtx     = mergeSynth1Contexts ctx (buildIntersectionBlastCtx inputs outputs)
  let safeCtx      = makeBlastCtxSafe blastCtx
  masks            <- Mask.computeMasksFn synthIntersectionBlastMasks (tacGoal {TacGoal.synthCtx = safeCtx})
  masks            <- liftIO $ Mask.uniqMasks masks
  candidates       <- Candidate.computeCandidatesFn synthIntersectionBlastCandidates (tacGoal {TacGoal.synthCtx = safeCtx})
