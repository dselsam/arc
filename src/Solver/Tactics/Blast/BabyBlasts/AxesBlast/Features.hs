...
{-# LANGUAGE StrictData #-}
module Solver.Tactics.Blast.BabyBlasts.AxesBlast.Features (synthAxesBlastMasks, synthAxesBlastCandidates) where
...
synthAxesBlastMasks :: StdGoal -> Synth1M (Ex (Grid Bool))
synthAxesBlastMasks goal@(Goal inputs outputs ctx) = do
  (masks :: Ex (Grid Bool)) <- choice "AxesBlast.synthMasks" [
    ("all", allMask goal)
...
synthAxesBlastCandidates :: StdGoal -> Synth1M (Ex (Grid Color))
synthAxesBlastCandidates goal@(Goal inputs outputs ctx) =
...
enumInt2Int ints =
  choice "int2int" [
    -- div and mod don't make sense for nInAxis and nDistinctInAxis
